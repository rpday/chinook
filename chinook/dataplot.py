...
class interface:
    ...
    def __init__(self,experiment):
        ...
        *args*:
        ...
        self.state_coords = np.array([self.axes[0][self.experiment.pks[:,1].astype(int)],self.axes[1][self.experiment.pks[:,2].astype(int)],self.experiment.pks[:,3]])
    ...
    def run_gui(self):
        ...
        self.slide_w = Slider(self.ax4,'',self.axes[0][0],self.axes[0][-1],valinit=self.axes[0][self.indx],valstep =self.deltas[self.dim])
        
        self.radio_ax = RadioButtons(self.ax5,('Momentum X','Momentum Y','Energy'),active=1)
        ...
        print(np.shape(self.Imat[self.indx,:,int(len(self.axes[ind1])/2)]),len(self.axes[ind2]))
        print(len(self.axes[ind1]),np.shape(self.Imat[self.indx,int(len(self.axes[ind2])/2),:]))
        self.rowdc, = self.ax2.plot(self.Imat[self.indx,:,int(len(self.axes[ind1])/2)],self.axes[ind2])
        self.coldc, = self.ax3.plot(self.axes[ind1],self.Imat[self.indx,int(len(self.axes[ind2])/2),:])
        self.find_cursor(img_centre)
        self.cursor_index = img_centre
        self.cursor, = self.ax1.plot([img_centre[0]],[img_centre[1]],marker='+',markersize=20,c='r')
        
       

        self.plot_peaks = self.state_coords[(ind1,ind2),:][:,np.where(self.state_coords[self.dim,:]==self.axes[self.dim][self.indx])[0]]
        self.bands, = self.ax1.plot(self.plot_peaks[0,:],self.plot_peaks[1,:],marker='o',linestyle='',markersize=2,c='w',alpha=0.2)
        ...
        def onclick(event):
            ...
            *args*:
            ...
            if event.inaxes==self.ax1.axes:
                ...
                self.find_cursor(coords)
#                self.cursor.set_data(np.array([coords[0],coords[1]]))
                self.fig.canvas.draw()
                plt.show()
    ...
    def find_cursor(self,cursor):
        ...
        *return*:
        ...
        axis1 = self.axes[ind2]
        axis2 = self.axes[ind1]
        
        row_index = np.where((abs(axis1-cursor[1])==abs(axis1-cursor[1]).min()))[0][0]
        col_index = np.where((abs(axis2-cursor[0])==abs(axis2-cursor[0]).min()))[0][0]
        
        print('row,',row_index)
        print('col',col_index)
        if self.dim == 0:
            rowdc = self.Imat[self.indx,:,col_index]
            coldc = self.Imat[self.indx,row_index,:]
        if self.dim == 1:
            rowdc = self.Imat[:,self.indx,col_index]
            coldc = self.Imat[row_index,self.indx,:]
        if self.dim == 2:
            rowdc = self.Imat[:,col_index,self.indx]
            coldc = self.Imat[row_index,:,self.indx]
        
        self.rowdc.set_data(rowdc,axis1)
        self.coldc.set_data(axis2,coldc)
        self.ax2.set_xlim(rowdc.min(),1.2*rowdc.max())
        self.ax3.set_ylim(coldc.min(),1.2*coldc.max())
            
        
        ### AXIS SCALING NEEDS TO BE FIXED
            

#        avail_states = np.where(self.state_coords[self.dim,:]==self.axes[self.dim][self.indx])[0]
#        closest = np.where(abs(self.state_coords[self.dim,:]-self.axes[self.dim][self.indx])==abs(self.state_coords[self.dim,:]-self.axes[self.dim][self.indx]).min())
#
...
#        self.cursor_index = avail_states[np.where(distance==distance.min())[0][0]]
        self.ax1.set_title('X: {:0.04f}, Y: {:0.04f}'.format(cursor[0],cursor[1]))
