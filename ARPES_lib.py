...
class experiment:
    ...
    def datacube(self,ARPES_dict):#cube,G,spin=None,T_eval = True, directory=None):
        ...
        if T_eval:
            fermi = np.array([con_ferm(self.Eb[i]/(kb*self.T/q)) for i in range(len(Eband_dig)) if dig_range[0]<=self.Eb[i]<=dig_range[-1]])
        else:
        ...
        for i in range(len(cube_indx)):
            if not ARPES_dict['slice'][0]:
                ...
                tmp_M = self.M_compute(Gvals,Bvals,i,cube_indx[i],tmp_basis,spin_mat,fermi[i],tol,strmats)
            elif abs(cube_indx[i][1]-ARPES_dict['slice'][1])<self.dE:
                tmp_M = self.M_compute(Gvals,Bvals,i,cube_indx[i],tmp_basis,spin_mat,fermi[i],tol,strmats)*np.exp(-(cube_indx[i][1]-ARPES_dict['slice'][1])**2/(2*self.dE))
            else:
                tmp_M = 0.0

#            sigma[i] = SE[0]+SE[1]*cube_indx[i,1]**2
            self.Mk[i,:,:] = tmp_M
...
#        self.write(Au,Ad,Eb,X,Y,directory,G)
    ...
    def M_compute(self,G,B,i,cube,basis,spinmat,fermi,tol,strmats):
...
#            
    ...
    def spectral(self,ARPES_dict):
        ...
        X,Y = np.meshgrid(x,y)
        w = np.linspace(*Ev)
        Kp,Ep=np.meshgrid(w,x)
        SE = [ARPES_dict['SE'][0]+p[2]**2*ARPES_dict['SE'][1] for p in self.pks]
        I = np.zeros((np.shape(X)[0],np.shape(X)[1],len(w)))
        
        for p in range(len(self.pks)):
            if abs(self.Mk[p].max())>0:
                Ev = int((self.pks[p,2]-w[0])/(w[1]-w[0]))
                I[int(np.real(self.pks[p,0])),int(np.real(self.pks[p,1])),Ev]+= (abs(np.dot(self.Mk[p,1,:],pol))**2 + abs(np.dot(self.Mk[p,1,:],pol))**2)*np.imag(-1./(np.pi*(w-self.pks[p,2]+0.02j)))
        
        fig = plt.figure()
        ax = fig.add_subplot(111)
        p = ax.pcolormesh(Kp,Ep,I[:,int(np.shape(X)[1]/2),:],cmap=cm.magma)
