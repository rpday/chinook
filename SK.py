...
def SK_coeff(o1,o2,Vcoeff):
    ...
    E = {}
    SK_co = 0.0
    rad = np.linalg.norm(o2.pos-o1.pos) #switched the two around 04/19/16 (o1 - o2 was old version)
    ...
    if o1.index != o2.index:   
        if abs(rad)< 0.001:
            ...
            l,m,n= np.array(o2.pos-o1.pos)/rad  #switched o1-o2 to o2-o1 04/20/16
            if l12=="00":
#                    osS = o1.orbital[:3]+o2.orbital[:3]+"S"
                if tmp+"S" in eta:
                    V = hb2m*eta[tmp+"S"]/(1.0)**2
#                        V = eta[tmp+"S"]
                elif tmp2+"S" in eta:
                   V = hb2m*eta[tmp2+"S"]/(1.0)**2
#                        V = eta[tmp2+"S"]
                else:
                    V = 0.0
                E["0-0"]= V
...
#                        V = eta[tmp+"S"]
                    ...
                    V = -1*hb2m*eta[tmp2+"S"]/(1.0)**2 #for Vps, need to use -1*Vsp
                else:
                    V = 0.0
                E["0-1x"],E["1x-0"] = [l*V]*2
                E["0-1y"],E["1y-0"] = [m*V]*2
                ...
                if tmp+"S" in eta:
                    V = [hb2m/(1.0)*eta[tmp+"S"],hb2m/(1.0)*eta[tmp+"P"]]
#                        V = eta[tmp+"S"],eta[tmp+"P"]
                elif tmp2+"S" in eta:
                    V = [hb2m/(1.0)*eta[tmp2+"S"],hb2m/(1.0)*eta[tmp2+"P"]]
...
#                    osP = o1.orbital[:3]+o2.orbital[:3]+"P"
                ...
                if tmp+"S" in eta:
                    V = [hb2m/((1.0)**2)*eta[tmp+"S"],hb2m/((1.0)**2)*eta[tmp+"P"]]
                elif tmp2+"S" in eta:
                    V = [hb2m/((1.0)**2)*eta[tmp2+"S"],hb2m/((1.0)**2)*eta[tmp2+"P"]]
                else:
                    V = [0.0,0.0]
